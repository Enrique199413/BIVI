...
{
	String codigo, titulo, tituloalt, asunto, autor, fechac, fechap, descripcion, duracion, formatodvid, formato, publicador, programa, rutaimgreal, rutaimg;
	Conexion objCon;
	Connection con;
	Statement stm;
	PrintWriter out;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException
	{
				...
				switch(j)
				{
					case 0:
						try
						{
							...
							rutaimgreal=codigo+"vid"+rutaimgreal;
							savedFile = new File(getServletContext().getRealPath("/")+"archivo", rutaimgreal);
							item.write(savedFile);
							out.println("<div class=\"naranja\">El video se ha enviado correctamente al servidor.</div>");
						}
						catch(Exception e)
						{
							rutaimgreal="";
							out.println("<div class=\"rojo\">El video no se ha enviado correctamente al servidor.</div>");
						}
					break;
					...
					case 1:
						try
						{
							...
							item.write(savedFile);
							out.println("<div class=\"naranja\">El thumbnail del video se ha enviado correctamente al servidor.</div>");
							out.println("<input type=\"button\" class=\"boton\" value=\"Generar thumbnail\" onclick=\"top.genThumbnailVid(\'"+rutaimg+"\')\" />");
						}
						catch(Exception e)
						{
							rutaimg="";
							out.println("<div class=\"rojo\">El thumbnail del no se ha enviado correctamente al servidor.</div>");
						}
					default:
		...
		try
		{
			stm.executeUpdate("INSERT INTO marticulo VALUES ('"+codigo+"', DEFAULT, 'archivo/"+rutaimg+"', "+formato+", "+publicador+", "+programa+", 4)");
			stm.executeUpdate("INSERT INTO mvideo VALUES ('"+codigo+"', '"+fechap+"', 'archivo/"+rutaimgreal+"', '"+titulo+"', '"+tituloalt+"', '"+descripcion+"', '"+asunto+"', '"+fechac+"', '"+autor+"', '"+duracion+"', "+formatodvid+")");
