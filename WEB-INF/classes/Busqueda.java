...
{
	String titulo, tituloalt, descripcion, editorial, edicion, tomo, asunto, autor, where;
	Conexion objCon;
	Connection con;  
	Statement stm;
	PrintWriter out;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException
	{
		...
		if(!tomo.equals(""))
		{
			where=where+"descripcion_tomo LIKE \"%"+tomo+"%\" AND ";
			boleano=true;
		}
		if(!asunto.equals(""))
		{
		...
		if(boleano)
		{
			...
			try
			{
				rs = stm.executeQuery("SELECT codigo_art, titulo_art, descripcion_art FROM mlibro LEFT JOIN ceditorial ON mlibro.id_editorial=ceditorial.id_editorial LEFT JOIN cedicion ON mlibro.id_edicion=cedicion.id_edicion LEFT JOIN ctomo ON mlibro.id_tomo=ctomo.id_tomo WHERE "+where);
				if(rs.next())
				{
					rs.beforeFirst();
					out.println("<html>");
