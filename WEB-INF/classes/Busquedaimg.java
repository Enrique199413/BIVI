...
public class Busquedaimg extends HttpServlet
{
	String titulo, tituloalt, descripcion, piefoto, asunto, autor, where;
	Conexion objCon;
	Connection con;  
	Statement stm;
	PrintWriter out;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException
	{
		...
		piefoto=request.getParameter("pief");
		...
		if(!piefoto.equals(""))
		{
			where=where+"pie_imagen LIKE \"%"+piefoto+"%\" AND ";
			boleano=true;
		}
		if(!asunto.equals(""))
		{
		...
		if(boleano)
		{
			...
			try
			{
				rs = stm.executeQuery("SELECT codigo_art, titulo_art, descripcion_art FROM mimagen WHERE "+where);
				if(rs.next())
				{
					rs.beforeFirst();
					out.println("<html>");
