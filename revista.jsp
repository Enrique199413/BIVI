...
<html>
	<head>
		...
		<script language="javascript" type="text/javascript" src="guion/revistagui.js"></script>
		</script>
			...
			<div id="som2">
				<div id="menu">
					<div id="cmenu">
						...
						<a href="revista.jsp"><div class="elemenu" id="seleccionado" >Revistas</div></a>
				...
				<div id="contenido">
					<div id="izquierdo">
						En este apartado podrá consultar revistas del Tribunal Federal de Justicia Fiscal y Administrativa.
						Las revistas se despliegan por su fecha de publicación a continuación:
						...
						<%
							...
							rsc = statem.executeQuery("SELECT COUNT(*) FROM mrevista");
							if(rsc.next())
							{
							...
							rsc = statem.executeQuery("SELECT * FROM marticulo RIGHT JOIN mrevista ON marticulo.codigo_art=mrevista.codigo_art ORDER BY fecha_publicacion DESC LIMIT 10");
							while(rsc.next())
							{
								out.println("<a href=\"articulo.jsp?codart="+rsc.getString("codigo_art")+"\">");
									out.println("<div class=\"unobra\">");
										...
										out.println("<div class=\"infoob\">");
											...
											rste = stateme.executeQuery("SELECT descripcion_edicion FROM cedicion WHERE id_edicion="+rsc.getString("id_edicion"));
											if(rste.next())
											{
												out.println("<span class=\"tit2\">Edición: </span>"+rste.getString("descripcion_edicion")+" edición<br>");
											}
											
											rste = stateme.executeQuery("SELECT descripcion_tomo FROM ctomo WHERE id_tomo="+rsc.getString("id_no_revista"));
											if(rste.next())
											{
						...
						<div id="checks">
							...
							<br>
							<!--[if gt IE 6]><center><![endif]-->
							<form method="post" action="Busquedarev">
								<table cellspacing="0" cellpadding="0" id="tabch">
									...
									<tr><td class="tder">Editorial:</td><td><input type="text" id="editorial" name="editorial" class="texto tatex" onkeyup="moPal('palo4', this.id)" onblur="moPal('palo4', this.id)" /></td><td><img src="img/paloma.png" id="palo4" style="display:none" /></td></tr>
									<tr><td class="tder">Edición:</td><td><input type="text" id="edicion" name="edicion" class="texto tatex" onkeyup="moPal('palo5', this.id)" onblur="moPal('palo5', this.id)" /></td><td><img src="img/paloma.png" id="palo5" style="display:none" /></td></tr>
									<tr><td class="tder">No. de revista:</td><td><input type="text" id="tomo" name="ndrev" class="texto tatex" onkeyup="moPal('palo6', this.id)" onblur="moPal('palo6', this.id)" /></td><td><img src="img/paloma.png" id="palo6" style="display:none" /></td></tr>
					...
					<div id="derecho">
						...
						<div id="elementos">
							<%
								...
								rs=stat.executeQuery("SELECT codigo_art, img_art FROM marticulo WHERE id_tipo=2 ORDER BY visitas_art DESC LIMIT 10");
								while(rs.next())
								{
									++i;
									codigo = rs.getString("codigo_art");
									rsn=state.executeQuery("SELECT titulo_art, descripcion_art, autor_art FROM mrevista WHERE codigo_art='"+codigo+"'");
